---
title: "Predicting Goal-to-go Offensive Play Call Proability"
author: "Jake Slaton"
output:
        html_document:
                toc: true
...
goal_offense <- pbp_raw %>% 
    ...
    mutate(
        down = as.factor(down),
        pass = factor(play_type, levels = c("run", "pass"))
    ) %>% 
    select(
        posteam,
        week,
...
## For this scenario, I am pretending we have only completed 17 weeks of the 2023
## season.
...
szn_epa <- pbp_raw %>%
    ...
    pivot_wider(
        ...
        names_prefix = "szn_epa_"
    )

## Visualize best and worst offenses
szn_epa %>% 
    ggplot(aes(x = szn_epa_pass, y = szn_epa_run)) +
    geom_text(aes(label = posteam)) +
    geom_smooth(method = "lm", se = FALSE)

## Combine season long EPA per play to our data set
...
goal_offense_wk_17 %>% 
    ...
    labs(
        title = "2023 Play Call Proportions By Down",
        x = "Down",
        y = "",
        fill = "Play Call"
    ) +
...
As may be assumed, during the 2023 season the proportion of pass plays called in goal-to-go situations increases as the down increases. It may also be important to consider where the line of scrimmage is located.
...
As may be expected, the probability of the play being a pass increases as the line of scrimmage becomes further from the goal-line. Something to consider, however, is how this may be conditional on the down.
...
The probability of pass, given the LOS, is conditional on the down. As seen in the plot, if the LOS is at the 5 yard line, the probability of a pass is ~35%, ~50%, ~75%, ~99% for 1st, 2nd, 3rd, and 4th down, respectively.
...
league_glm <- stan_glm(pass ~ yardline_100 + down + yardline_100:down,
                       data = goal_offense_wk_17[train_obs,],
...
league_pred <- posterior_epred(league_glm,
                               newdata = goal_offense_wk_17[test_obs,])
...
league_roc <- pROC::roc(goal_offense_wk_17$pass[test_obs],
                        apply(league_pred, 2, median))
pROC::auc(league_roc)
```

An AUC > 0.7 is an acceptable level of discrimination. Essentially, the model may do an acceptable job of correctly classifying the play call. The goal here is to _aid_ the defensive coordinator, therefore an acceptable level of classification is likely still useful. However, knowing the predicted probability for a given situation is more useful than dichotomizing the information.
...
team_pred <- posterior_epred(var_int,
                             newdata = goal_offense_wk_17[test_obs,])

## Calculate ROC
team_roc <- pROC::roc(goal_offense_wk_17$pass[test_obs],
                      apply(team_pred, 2, median))
...
## Plot visualizing intercept error terms
lattice::dotplot(ranef(grp_lvl_variable, which = "posteam", condVar = TRUE))
